    ...
    <div className="product-frame relative overflow-hidden rounded-2xl">
      ...
      <div className="flex items-center justify-between border-b border-border/60 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-heading-muted/90">Hermes · Online</span>
        </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-3 border-b border-border/50 p-4 lg:border-b-0 lg:border-r">
          <div className="max-w-[90%] rounded-xl border border-border/60 bg-muted px-3 py-2.5 text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {["Update my CV", "Find matches", "Publish page"].map((label) => (
              <span
                key={label}
                className="rounded-md border border-border/70 bg-muted/40 px-2.5 py-1 text-[11px] text-heading-muted/80"
              >
                {label}
              </span>
            ))}
        ...
        <div className="relative space-y-2.5 overflow-hidden p-4">
          ...
          <div className="relative flex items-center justify-between">
            <p className="text-xs font-medium text-heading-muted">Top matches</p>
            <p className="text-[11px] text-muted-foreground">Live</p>
          </div>
          {topJobs.map((job, index) => (
            <div
              key={job.id}
              className="rounded-lg border border-border/60 bg-card/90 px-3 py-2.5"
            >
...
export function PipelineVisual({
  ...
  if (step === "find") {
    return (
      <div className="product-frame relative overflow-hidden rounded-xl p-4 font-mono text-[11px] leading-relaxed text-heading-muted/70">
  ...
  if (step === "prep") {
    return (
      <div className="product-frame space-y-2 rounded-xl p-4 text-sm">
        ...
        <p className="rounded-md bg-red-500/10 px-2 py-1.5 text-red-700/90 line-through decoration-red-400/50 dark:text-red-200/90 dark:decoration-red-300/40">
      ...
      <div className="product-frame space-y-2 rounded-xl p-4">
        {listCatalogJobs().slice(0, 2).map((job, i) => (
          <div key={job.id} className="flex items-center justify-between gap-3 text-sm">
            <div className="min-w-0">
              <p className="truncate font-medium text-heading">{job.title}</p>
  ...
  return (
    <div className="product-frame space-y-3 rounded-xl p-4 text-sm">
      <div className="flex items-center justify-between">
        <span className="text-heading">Mobilization checklist</span>
  ...
  return (
    <div className="product-frame overflow-hidden rounded-2xl">
      <div className="flex items-center gap-2 border-b border-border/60 px-4 py-3">
        ...
        <div>
          <p className="text-sm font-medium text-heading">Hermes</p>
          <p className="text-[11px] text-muted-foreground">Your commercial diving agent</p>
        </div>
      </div>
      <div className="space-y-3 p-4">
        <div className="ml-auto max-w-[85%] rounded-2xl rounded-br-md bg-chat-user px-3.5 py-2.5 text-sm text-heading">
          Add a 2024 North Sea IRM campaign to my CV and rewrite the summary in English.
        </div>
        <div className="max-w-[90%] rounded-2xl rounded-bl-md border border-border/60 bg-muted px-3.5 py-2.5 text-sm text-heading/95">
        ...
        <div className="rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5">
          ...
          <p className="text-sm font-medium text-heading">Offshore Wind Inspection Team</p>
